...
function print_my_ip_info() {
    ...
    local include_mac=0           # MAC 수집 플래그
    local debug_mode=0
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --include-ipv6|--all) include_ipv6=1 ;;
            --include-mac)     include_mac=1     ;;
            --nic)
                shift
...
Options:
  ...
  --include-ipv6, --all                     IPv4 + IPv6 모두 출력합니다.
  --include-mac                             MAC 주소도 함께 출력합니다.
  --nic <iface1,iface2>                    특정 NIC만 필터링해서 출력합니다.
  --show-ifname-peer                        인터페이스 이름에 @peer 정보 포함.
...
Examples:
  ...
  $cmdname --include-mac -o markdown
...
EOF
    ...
    if command -v ip >/dev/null 2>&1; then
        [[ $debug_mode -eq 1 ]] && echo "[DEBUG] Using 'ip' for interface detection." >&2
        ...
        [[ $debug_mode -eq 1 ]] && echo "[DEBUG] Using 'ifconfig' for interface detection." >&2
    ...
    # 1.1) MAC 수집 (옵션으로 활성화)
    if [[ $include_mac -eq 1 ]]; then
        [[ $debug_mode -eq 1 ]] && echo "[DEBUG] Collecting MAC addresses." >&2
        for iface in "${up_interfaces[@]}"; do
            ...
            add_nic_entry "$iface" "mac" "$mac"
            [[ $debug_mode -eq 1 ]] && echo "[DEBUG] MAC $iface=$mac" >&2
        done
    fi

    # 2) IP 수집
    if command -v ip >/dev/null 2>&1; then
        [[ $debug_mode -eq 1 ]] && echo "[DEBUG] Using 'ip' for IP collection." >&2
        ...
        [[ $debug_mode -eq 1 ]] && echo "[DEBUG] Using 'ifconfig' for IP collection." >&2
        local current_iface=""
        while IFS= read -r line; do
